...
public class LocalImageLoader {
	...
	private final String DEFAULT_TAG = "Ddefault_photo_tag";
	 ...
	 * @param imageView
	 */
	public void loadImage(final Context context,final String singerName, final ImageView imageView) {

		// set tag
		final String tag = Utils.Md5(singerName);
		...
		Bitmap bm = getBitmapFromLruCache(tag);
		if (bm != null) {
			...
			final String filepath = context.getFilesDir().getAbsolutePath()+"/"+"portraitThumbnail"+"/"+singerName+".jpg";
			if (!new File(filepath).exists()) {
				Bitmap default_bm = getBitmapFromLruCache(DEFAULT_TAG);
				if (default_bm != null) {
					ImgBeanHolder holder = new ImgBeanHolder();
					holder.bitmap = default_bm;
					holder.imageView = imageView;
					holder.path = tag;
					Message message = Message.obtain();
					message.obj = holder;
					...
					addTask(new Runnable() {
						...
						public void run() {
							...
							Bitmap bm = decodeSampledBitmapFromId(context, R.drawable.default_singer, reqWidth, reqHeight);
							addBitmapToLruCache(DEFAULT_TAG, bm);
							ImgBeanHolder holder = new ImgBeanHolder();
							holder.bitmap = getBitmapFromLruCache(DEFAULT_TAG);
							holder.imageView = imageView;
							holder.path = tag;
							Message message = Message.obtain();
							message.obj = holder;
				...
				addTask(new Runnable() {
					...
					public void run() {
						...
						addBitmapToLruCache(tag, bm);
						ImgBeanHolder holder = new ImgBeanHolder();
						holder.bitmap = getBitmapFromLruCache(tag);
						holder.imageView = imageView;
						holder.path = tag;
						Message message = Message.obtain();
						message.obj = holder;
	...
	private Bitmap decodeSampledBitmapFromId(Context context ,int  id, int reqWidth, int reqHeight) {
		// 第一次解析将inJustDecodeBounds设置为true，来获取图片大小
		final BitmapFactory.Options options = new BitmapFactory.Options();
		options.inJustDecodeBounds = true;

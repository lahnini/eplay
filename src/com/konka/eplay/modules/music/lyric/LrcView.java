package com.konka.eplay.modules.music.lyric;
...
public class LrcView extends View implements ILrcView {
	...
	/** 高亮歌词的默认字体颜色 **/
	private static final int DEFAULT_COLOR_FOR_OTHER_LRC = 0xffa7a7a7;
	/** 高亮歌词当前的字体颜色 **/
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mLrcRows == null || mLrcRows.size() == 0) {
			...
			// mPaintForOtherLrc);
			canvas.drawText(DEFAULT_TEXT, 0, getHeight() / 2, mPaintForOtherLrc);
			return;
		}
		if (mTotleDrawRow == 0) {
			// 初始化将要绘制的歌词行数
		...
		for (int i = minRaw; i <= maxRaw; i++) {
			...
			if (i == mCurRow) {// 画高亮歌词
				...
				if (textWidth > getWidth()) {
					...
					float textX = (getWidth() - textWidth) / 2;
					// canvas.drawText(text, textX, rowY,
				...
				mPaintForOtherLrc.setColor(0x1000000 * curAlpha + 0xa7a7a7);
				// canvas.drawText(text, textX, rowY, mPaintForOtherLrc);
				canvas.drawText(text, 0, rowY, mPaintForOtherLrc);
			}
			// 计算出下一行歌词绘制的y坐标
		...
		case MotionEvent.ACTION_CANCEL:
			if (!canDrag) {
				...
				if (getScrollY() < 0) {
					...
					smoothScrollTo((int) (mLrcRows.size() * (mCurSizeForOtherLrc + mCurPadding) - mCurPadding),
	...
	public void seekTo(int progress, boolean fromSeekBar, boolean fromSeekBarByUser) {
		...
		for (int i = mLrcRows.size() - 1; i >= 0; i--) {
			...
			//		Trace.Info("textWidth=" + textWidth + "getWidth()=" + getWidth());
					if (textWidth > getWidth()) {
						if (fromSeekBarByUser) {
							mScroller.forceFinished(true);
						}
					//	Trace.Info("开始水平滚动歌词:" + mLrcRows.get(mCurRow).getContent());
						startScrollLrc(getWidth() - textWidth, (long) (mLrcRows.get(mCurRow).getTotalTime() * 0.6));

						long pasttime = progress - mLrcRows.get(mCurRow).getTime();
					//	Trace.Info("passtime=" + pasttime + "  totaltime=" + mLrcRows.get(mCurRow).getTotalTime());

						// startScrollLrc(getWidth() - textWidth,
						// (long) ((mLrcRows.get(mCurRow).getTotalTime() -
						// pasttime) * 0.6));
					}
					invalidate();
				}
	...
	private void startScrollLrc(float endX, long duration) {
		...
		if (duration>0) {
			...
			mAnimator.setStartDelay((long) (1000 * 0.3)); // 延迟执行属性动画
		}
	...
	AnimatorUpdateListener updateListener = new AnimatorUpdateListener() {
		...
		public void onAnimationUpdate(ValueAnimator animation) {
			...
			// Trace.Info("mCurTextXForHighLightLrc=" +
	...
	public void setLrcScalingFactor(float scalingFactor) {
		...
		// mTotleDrawRow = (int) (getHeight() / (mCurSizeForOtherLrc +
		// mCurPadding)) + 3;
		Trace.Info("mRowTotal=" + mTotleDrawRow);
		scrollTo(getScrollX(), (int) (mCurRow * (mCurSizeForOtherLrc + mCurPadding)));
		invalidate();
		mScroller.forceFinished(true);
	}

...
public class Vitamio {
	private static final String[] LIBS_ARM_CODECS = { "libvvo.7.so",
					"libvvo.8.so", "libffmpeg.so", "libOMX.9.so",
					"libOMX.11.so", "libOMX.14.so", "libOMX.18.so" };
	private static final String[] LIBS_X86_CODECS = { "libffmpeg.so",
					"libOMX.9.so", "libOMX.14.so", "libOMX.18.so" };
	private static final String[] LIBS_MIPS_CODECS = { "libffmpeg.so",
					"libOMX.14.so" };
	private static final String[] LIBS_PLAYER = { "libvplayer.so" };
	private static final String[] LIBS_SCANNER = { "libvscanner.so" };
	private static final String[] LIBS_AV = { "libvao.0.so", "libvvo.0.so",
	...
	public static boolean initialize(Context ctx, int rawId) {
		boolean ret = isInitialized(ctx) || extractLibs(ctx, rawId);
		Trace.Debug("###initialize() return " + ret);
		return ret;
	}
	...
	public static boolean isInitialized(Context ctx) {
		...
		Trace.Debug("###vitamioLibraryPath=" + vitamioLibraryPath);
		File dir = new File(getLibraryPath());
		if (dir.exists() && dir.isDirectory()) {
			String[] libs = dir.list();
			if (libs != null) {
				Arrays.sort(libs);
				Log.e("###vitamioType = " + vitamioType);
				...
				try {
					...
					Log.i("isNativeLibsInited, APP VERSION: %d, Vitamio Library version: %d",
	...
	private static final List<String> getRequiredLibs() {
		...
		case VITAMIO_ARMV7_NEON:
			vitamioLibs = new String[][] { LIBS_ARM_CODECS, LIBS_PLAYER,
							LIBS_SCANNER, LIBS_AV };
			break;
		case VITAMIO_X86:
			vitamioLibs = new String[][] { LIBS_X86_CODECS, LIBS_PLAYER,
							LIBS_SCANNER, LIBS_AV };
			break;
		case VITAMIO_MIPS:
			vitamioLibs = new String[][] { LIBS_MIPS_CODECS, LIBS_PLAYER,
							LIBS_SCANNER, LIBS_AV };
			break;
		default:
			break;
	...
	private static boolean extractLibs(Context ctx, int rawID) {
		...
		Log.d("copyCompressedLib time: " + (System.currentTimeMillis() - begin)
						/ 1000.0);
		boolean inited = native_initializeLibs(libPath, getLibraryPath(),
						String.valueOf(Vitamio.getVitamioType()));
		...
		try {
			...
			Log.d("loadLibs time: " + (System.currentTimeMillis() - begin)
							/ 1000.0);
	...
	private native static boolean native_initializeLibs(String libPath,
